...
// for (int i = 1; i < n; i++) { // Change `<=` to `<` to avoid
...
// for (int i = n - 2; i >= 0; i--) { // Iterate correctly from `n-2` down to
// `0`
// rightmax[i] = Math.max(height[i], rightmax[i + 1]);
// }
...
// for (int i = 0; i < n; i++) {
// totalTrapedwater += Math.min(leftmax[i], rightmax[i]) - height[i]; // Fix
// subtraction
// }
...
// public static void main(String[] args) {
// int height[] = { 4, 2, 0, 6, 3, 2, 5 };
// System.out.println(trappedRainwater(height)); // Should print the trapped
// water
// }
// }
